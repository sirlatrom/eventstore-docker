...
#RUN apt-get update && apt-get install -y curl sudo && apt-get autoremove --purge -y && rm -rf /var/lib/apt/lists/*
ENV EVENTSTORE_VERSION=3.3.0
#RUN curl http://download.geteventstore.com/binaries/EventStore-OSS-Ubuntu-v$EVENTSTORE_VERSION.tar.gz | tar xz -C /opt
